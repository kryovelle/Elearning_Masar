...
<div class="auth-page">
    <div class="auth-card">
        <div class="auth-header">
            <span class="eyebrow" data-i18n="login_eyebrow">Welcome back</span>
            <h1 data-i18n="login_title">Log in to your account</h1>
            <p data-i18n="login_subtitle">Enter your email and password to continue.</p>
        </div>

        <div id="loginError" class="auth-error" style="display:none;"></div>

        <form id="loginForm" class="auth-form">
            <label for="email" data-i18n="login_email_label">Email</label>
            <input type="email" id="email" required placeholder="you@example.com">

            <label for="password" data-i18n="login_password_label">Password</label>
            <input type="password" id="password" required placeholder="••••••••">

            <button id="submitBtn" type="submit" class="btn-auth" data-i18n="login_submit">Log in</button>
        </form>

        <p class="auth-footer-text">
            <span data-i18n="login_no_account">Don't have an account?</span>
            <a href="/Public/Register" data-i18n="login_register_link">Register</a>
        </p>
    </div>
</div>
@endsection
...
async function postlogin(event) {
    ...
    try {
        ...
        const res = await axios.post('/api/Public/postLogin',{ email, password });
...
async function verifyMfa(event) {
    ...
    try {
        const res =await axios.post('/api/Public/mfa/verify',{ mfa_token, code });
...
async function verifyRecoveryCode(event) {
    ...
    try {
        ...
        const res= await axios.post('/api/Public/verifyMfaWithRecoveryCode',{ mfa_token, recovery_code })
        const data = res.data;
